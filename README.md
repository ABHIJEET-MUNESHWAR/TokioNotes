...
3. [🏛 Architecture](#-architecture)
   ...
   - [Components — What / How / Why](#components--what--how--why)
     ...
     - [Layer 2 — API / Gateway (`crates/gateway`)](#layer-2--api--gateway-cratesgateway)
     - [Layer 3 — Application / Services](#layer-3--application--services)
     - [Layer 4 — Domain (`crates/domain`)](#layer-4--domain-cratesdomain)
...
8. [🛠 How It Works](#-how-it-works)
   ...
   - [External flows (GraphQL API)](#external-flows-graphql-api)
     - [1. `register`](#1-registeremail-displayname-password--mutation)
     - [2. `login`](#2-loginemail-password--mutation)
     - [3. `me`](#3-me--query)
     - [4. `createNote`](#4-createnotetitle--mutation)
     - [5. `renameNote`](#5-renamenoteid-title--mutation)
     - [6. `deleteNote`](#6-deletenoteid--mutation)
     - [7. `shareNote` (Saga)](#7-sharenoteid-email-role--mutation-saga)
     - [8. `revokeShare`](#8-revokeshareid-userid--mutation)
     - [9. `myNotes`](#9-mynotes--query)
     - [10. `note` / `collaborators`](#10-noteid--collaboratorsid--query)
     - [11. `applyOps` (CRDT hot path)](#11-applyopsnoteid-updateb64--mutation-crdt-hot-path)
     - [12. `noteOps` (CRDT subscription)](#12-noteopsnoteid--subscription-crdt-read-path)
     - [13. `aiSummary` / `aiReport`](#13-aisummaryid--aireportid-instruction--query)
   - [Internal / background flows](#internal--background-flows)
     - [A. Event emission → bus → consumers](#a-event-emission--bus--consumers)
     - [B. `RoomManager` lifecycle](#b-roommanager-lifecycle)
     - [C. Resilience combinators](#c-resilience-combinators-tn-commonresilience)
     - [D. Sharding / partitioning](#d-sharding--partitioning)
