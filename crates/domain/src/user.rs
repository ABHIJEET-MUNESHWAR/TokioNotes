...
impl UserBuilder {
    pub fn id(mut self, v: UserId) -> Self {
        self.id = Some(v);
        self
    }
    ...
    pub fn password_hash(mut self, v: impl Into<String>) -> Self {
        self.password_hash = Some(v.into());
        self
    }
    ...
    pub fn build(self) -> AppResult<User> {
        ...
        let display_name = self
            .display_name
            .ok_or_else(|| AppError::Validation("display_name required".into()))?;
        if display_name.trim().is_empty() {
            return Err(AppError::Validation("display_name empty".into()));
        }
        let password_hash = self
            .password_hash
            .ok_or_else(|| AppError::Validation("password_hash required".into()))?;
        Ok(User {
